...
/* ------------------------------------------------------------------ */
/*  NodeStateCounts — counters per node state                          */
/*                                                                     */
/*  Week 2 deliverable: counts are derived from live Redux node        */
/*  telemetry, so they update as WebSocket events arrive.              */
/* ------------------------------------------------------------------ */
...
const ENTRIES: StateCountEntry[] = [
  { state: "CHARGING", label: "Charging", icon: "🔋", color: "text-green-400", barColor: "bg-green-500" },
  { state: "DISCHARGING", label: "Discharging", icon: "⚡", color: "text-orange-400", barColor: "bg-orange-500" },
  { state: "IDLE", label: "Idle", icon: "⏸", color: "text-slate-400", barColor: "bg-slate-500" },
  { state: "SOLAR", label: "Solar", icon: "☀", color: "text-yellow-400", barColor: "bg-yellow-500" },
  { state: "FAULT", label: "Fault", icon: "⚠", color: "text-red-400", barColor: "bg-red-500" },
];
...
const NodeStateCounts: React.FC = () => {
  ...
  /* Count each state in a single pass (avoids 5 x filter() over 50k nodes) */
          ...
          return (
            <div key={entry.state}>
              <div className="flex items-center justify-between text-xs">
                <span className={`flex items-center gap-1.5 ${entry.color} font-medium`}>
                  <span>{entry.icon}</span>
                  <span>{entry.label}</span>
                </span>
                <span className="font-mono text-slate-200">
                  {count.toLocaleString()}
                  <span className="text-slate-500 ml-1">({share}%)</span>
                </span>
              </div>
              {/* Mini bar chart */}
